...
nocache("./command/case.js", (module) =>
  console.log(`${module} is now updated!`)
);

const starts = async (haruka = new WAConnection()) => {
  haruka.logger.level = "warn";
  haruka.version = [2, 2143, 3];
  haruka.browserDescription = ["Pogam 2.0", "Chrome", "3.0"];
  CFonts.say("Pogam", {
    font: "block",
    color: ["#ff9c00"],
    align: "center",
  });
  CFonts.say(`Bot WhatsApp Created By Nivth`, {
    font: "console",
    align: "center",
  ...
  haruka.on("qr", () => {
    console.log(
      ...
      color("!", "red"),
      color("]", "white"),
      color(" Scan qr maks 20 detik sebelum qr expired")
    );
  });
  ...
  fs.writeFileSync(
    "./session.json",
    JSON.stringify(haruka.base64EncodedAuthInfo(), null, "\t")
  );
  ...
  haruka.on("group-participants-update", async (anu) => {
    ...
    try {
      const sendButLoc = async (
        ...
        const mediaxxaaaa = await haruka.prepareMessage(
          id,
          gam1,
          MessageType.location,
          { thumbnail: gam1 }
        );
        var mhan = mediaxxaaaa.message["ephemeralMessage"]
          ? mediaxxaaaa.message.ephemeralMessage
          : mediaxxaaaa;
      ...
      if (anu.action == "add" && !num.includes(haruka.user.jid)) {
        welcome = await getBuffer(
          `https://api-alphabot.herokuapp.com/api/greetings/welcome2?name=${encodeURI(
            anu_user
          )}&member=${encodeURI(
            mdata.participants.length
          )}&groupName=${encodeURI(
            mdata.subject
          )}&ppuser=${ppmem3}&bgurl=${background}&apikey=Alphabot`
        );
        try {
          await sendButLoc(
            mdata.id,
        ...
        goodbye = await getBuffer(
          `https://api-alphabot.herokuapp.com/api/greetings/goodbye2?name=${encodeURI(
            anu_user
          )}&member=${encodeURI(
            mdata.participants.length
          )}&groupName=${encodeURI(
